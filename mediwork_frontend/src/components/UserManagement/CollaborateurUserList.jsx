...
export default function CollaborateurUserList() {
  ...
  const loadCollaborateurUsers = async () => {
    setLoading(true);
    try {
      const collaborateurUsers = await userApi.getCollaborateurUsers();
      setUsers(collaborateurUsers);
    } catch (error) {
  ...
  useEffect(() => {
    loadCollaborateurUsers();
  }, []);

  // CRUD Operations - UPDATED with proper error handling
  const handleAddUser = async () => {
    if (!formData.firstName || !formData.lastName || !formData.email || !formData.matricule || !formData.password) {
    ...
    try {
      ...
      setFormData({ firstName: '', lastName: '', email: '', matricule: '', password: '', role: 'COLLABORATOR' });
      loadCollaborateurUsers(); // Refresh the list
  ...
  return (
    <PageWrapper title="Collaborateurs" className="p-6">
      <section className="mx-auto w-full max-w-7xl px-4 py-4">
        {/* En-tête */}
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold">Liste des collaborateurs</h2>
          <div className="flex space-x-2">
            <button
              onClick={loadCollaborateurUsers}
              disabled={loading}
        ...
        <div className="overflow-x-auto border border-gray-300 rounded-xl shadow-lg bg-gradient-to-br from-white to-gray-50">
          <table className="min-w-full text-sm">
            <thead className="sticky top-0 z-10 bg-blue-50">
              <tr>
                <th className="px-6 py-3 font-semibold text-blue-900 text-left border-b border-blue-200 rounded-tl-xl">Infos Collaborateur</th>
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-blue-50 bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl shadow-md shadow-gray-200 p-8 w-full max-w-md border border-gray-100">
              <h3 className="text-2xl font-medium text-gray-800 mb-6 text-center">Ajouter un collaborateur</h3>
        ...
        {showEditModal && (
          <div className="fixed inset-0 bg-blue-50 bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl shadow-md shadow-gray-200 p-8 w-full max-w-md border border-gray-100">
              <h3 className="text-2xl font-medium text-gray-800 mb-6 text-center">Modifier le collaborateur</h3>
              <div className="space-y-5">
                <input
                  type="text"
                  name="firstName"
